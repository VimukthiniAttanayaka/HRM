...
//using HRM_DAL.DAL.Dtos.Invoice;
...
namespace HRM_DAL.DAL.Helper
{
    ...
    public class PageList<T> : List<T>
    {
        ...
        {
            var count = 0;// await source.CountAsync();
            var items = new List<T>();// await source.Skip((pageNumber - 1) * pageSize).Take(pageSize).ToListAsync();
