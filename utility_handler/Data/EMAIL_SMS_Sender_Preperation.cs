    ...
    public class EMAIL_SMS_Sender_Preperation
    {
        internal static GetSMS_BusinessUnitModel GetSMS_BusinessUnit(string BU_ID)
        {
            ...
            try
            {
                ...
                DayEndProcess_Data.WriteLog("GetSMS_BusinessUnit", "modify_sk_user_access_group - BU request read" + logobject);
            }
            catch (Exception ex)
            {
                DayEndProcess_Data.WriteLog("GetSMS_BusinessUnit", "Fail " + ex.Message + " - " + ex.StackTrace);
            }
            return ump;
        }
