...
import { modifyCustomer } from '../../../apicalls/company/modify.js';
import { deleteCustomer } from '../../../apicalls/company/delete.js';
import { addNewCustomer } from '../../../apicalls/company/add_new.js';
...
const CompanyPopup = ({ visible, onClose, onOpen, CompanyDetails, popupStatus }) => {
  ...
  const handleChangeId = (event) => { setCustomerId(event.target.value) }
  const handleChangeName = (event) => { setCompanyName(event.target.value) }
  const handleChangeGroupName = (event) => { setGroupName(event.target.value) }
  const handleChangeBlockBuildingNumber = (event) => { setBlockBuildingNumber(event.target.value) }
  const handleChangeAddressBuildingName = (event) => { setAddressBuildingName(event.target.value) }
  const handleChangeAddressUnitNumber = (event) => { setAddressUnitNumber(event.target.value) }
  const handleChangeAddressStreetName = (event) => { setAddressStreetName(event.target.value) }
  const handleChangeAddressCity = (event) => { setAddressCity(event.target.value) }
  const handleChangeAddressCountryCode = (event) => { setAddressCountryCode(event.target.value) }
  const handleChangeAddressPostalCode = (event) => { setAddressPostalCode(event.target.value) }
  const handleChangeContactPerson = (event) => { setContactPerson(event.target.value) }
  const handleChangeContactNumber = (event) => { setContactNumber(event.target.value) }
  const handleChangePinOrPassword = (event) => { setPinOrPassword(event.target.value) }
  const handleChangeIsOtpSms = (event) => { setIsOtpSms(event.target.checked) }
  const handleChangeIsOtpEmail = (event) => { setIsOtpEmail(event.target.checked) }
  ...
  const handleSubmit = async (event) => {
    ...
    if (popupStatus == 'edit') {
      ...
      const APIReturn = await deleteCustomer(formData)
      if (APIReturn.resp === false) { setDialogTitle("Alert"); }
      else { setDialogTitle("Message"); }
      setDialogContent(APIReturn.msg);
      setOpen(true);
      ...
      const APIReturn = await addNewCustomer(formData)
      if (APIReturn.resp === false) { setDialogTitle("Alert"); }
      else { setDialogTitle("Message"); }
          ...
          <CCard className="mx-4">
            <CCardBody className="p-4">
              ...
              <CForm onSubmit={handleSubmit}>
                {/* <h1>{popupStatus == 'edit' ? "Edit" : "Add"}</h1>
                <p className="text-body-secondary">{popupStatus == 'edit' ? "Edit" : "Add"} Your Company Details</p> */}
