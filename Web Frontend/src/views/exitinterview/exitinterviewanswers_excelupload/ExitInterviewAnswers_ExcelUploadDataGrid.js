...
const ExitInterviewAnswers_ExcelUploadDataGrid = () => {
  ...
  const [ExitInterviewAnswersDetails, setExitInterviewAnswersDetails] = useState([])
  ...
  async function loadDetails(item, action) {
    ...
    const formData = {
      ...
      EME_ExitInterviewAnswersID: item
    }
    // const ExitInterviewAnswersDetails = await getExitInterviewAnswersSingle(formData)
    // setExitInterviewAnswersDetails(ExitInterviewAnswersDetails);
    handleOpenPopup()
  }
  ...
  async function requestdata() {
    ...
    const formData = {
      // UD_StaffID: staffId,
      // AUD_notificationToken: token,
      EME_ExitInterviewAnswersID: 'sedcx'
    }

    // const ExitInterviewAnswersDetails = await getExitInterviewAnswersAll(formData)
    // setData(ExitInterviewAnswersDetails);
  ...
  const handleClosePopup = () => {
    setVisible(false);
    setExitInterviewAnswersDetails([]);
        ...
        <CCol>
          <CDropdown>
            ...
            <CDropdownMenu>
              ...
              <CDropdownItem>
                <CSmartGridPDF data={data} headers={headers} filename="exitinterviewquestions" title="exitinterviewquestions" />
              </CDropdownItem>
            </CDropdownMenu>
          </CDropdown>
        </CCol>
        <CCol className='d-flex justify-content-end'>
          {/* <ExitInterviewAnswersPopupTab popupStatus={popupStatus} onClose={handleClosePopup} visible={visible} onOpen={handleOpenPopup} ExitInterviewAnswersDetails={ExitInterviewAnswersDetails} /> */}
        </CCol>
      </CRow>
      <CSmartTable
        cleaner
            ...
            return (
              <td className="py-2">
                <CButton
                  ...
                  size="sm"
                  onClick={() => {
                    toggleEdit(item.EME_ExitInterviewAnswersID)
                  }}
                >
                  Edit
                </CButton>
          ...
          view: (item) => (
            <td>
              <CButton
                ...
                size="sm"
                onClick={() => {
                  toggleView(item.EME_ExitInterviewAnswersID)
                }}
              >
                View
              </CButton>
            ...
            <td>
              {item.status == 'Inactive' ? '' :
                <CButton
                  ...
                  size="sm"
                  onClick={() => {
                    toggleDelete(item.EME_ExitInterviewAnswersID)
                  }}
                >
                  Delete
                </CButton>
...
export default ExitInterviewAnswers_ExcelUploadDataGrid
